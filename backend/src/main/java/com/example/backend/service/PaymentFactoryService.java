...
public class PaymentFactoryService {
    ...
    public PaymentResponse createPayment(PaymentRequest request) {
        return switch (request.paymentMethod()) {
            ...
            case BANK_TRANSFER, CASH, CREDIT_CARD -> throwUnsupportedPaymentMethod(request.paymentMethod());
        };
    }
    ...
    public PaymentResponse processCallback(PaymentMethod method, String transactionCode, String signature, Object callbackData) {
        return switch (method) {
            case MOMO -> momoPaymentService.processCallback(method, transactionCode, signature, callbackData);
            case VNPAY -> vnpayPaymentService.processCallback(method, transactionCode, signature, callbackData);
            case BANK_TRANSFER, CASH, CREDIT_CARD -> throwUnsupportedPaymentMethod(method);
        };
    }

    public PaymentResponse refundPayment(PaymentMethod method, String transactionCode, String reason) {
        return switch (method) {
     ...
     * 
     * @param method The unsupported payment method
     * @return Never returns, always throws exception
     * @throws BusinessException with appropriate error message
     */
